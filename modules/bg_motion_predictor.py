...
class BGMotionPredictor(nn.Module):
    ...
    def __init__(self, block_expansion, num_channels, max_features, num_blocks, bg_type='zero', translation_type='none'):
    ...
    def forward(self, source_image, driving_image):
        ...
        if self.bg_type != 'zero':
            ...
            if self.bg_type == 'shift':
                ...
                prediction  = prediction.view(bs, 2, 3)
                if self.translation_type == 'sigmoid':
                    prediction[:,:,2:] = 4 * torch.sigmoid(prediction[:,:,2:]) - 2
                else:
                    assert self.translation_type == 'none'
                out[:, :2, :] = prediction
