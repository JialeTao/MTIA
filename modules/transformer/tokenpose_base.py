...
class Attention(nn.Module):
    def __init__(self, dim, heads = 8, dropout = 0., num_keypoints=None, scale_with_head=False, fix_img2motion_attention=False, num_img_tokens=None):
        super().__init__()
        self.heads = heads
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, mlp_dim, dropout,num_keypoints=None,all_attn=False, scale_with_head=False, fix_img2motion_attention=False, num_patches=256):
        super().__init__()
        ...
        for d in range(depth):
            ...
            self.layers.append(nn.ModuleList([
                Residual(PreNorm(dim, Attention(dim, heads = heads, dropout = dropout, num_keypoints=num_keypoints, scale_with_head=scale_with_head, fix_img2motion_attention=fix_img2motion_attention,num_img_tokens=num_patches)), num_keypoints=self.num_keypoints),
...
class TokenPose_TB_base(nn.Module):
    def __init__(self, *, feature_size, patch_size, num_keypoints, dim, depth, heads, mlp_dim, apply_init=False, apply_multi=True, hidden_heatmap_dim=64*6,heatmap_dim=64*64,heatmap_size=[64,64], channels = 3, dropout = 0., emb_dropout = 0., pos_embedding_type="sine-full",
                 estimate_jacobian=True, temperature=0.1, spatial_kp_head=False, jacobian_token=True, hidden_dim=False, affine_jacobian=False, fix_img2motion_attention=False):
        super().__init__()
        ...
        # transformer
        self.transformer = Transformer(dim, depth, heads, mlp_dim, dropout,num_keypoints=num_keypoints,all_attn=self.all_attn, scale_with_head=True, fix_img2motion_attention=fix_img2motion_attention, num_patches=num_patches)
        ...
        if estimate_jacobian:
            ...
            self.mlp_head_jacobian[-1].bias.data.copy_(torch.tensor([1,0,0,1], dtype=torch.float))
    ...
    def forward(self, feature, mask = None):
        ...
        if self.spatial_kp_head:
            ...
            heatmap = rearrange(heatmap,'b c (p1 p2) -> b c p1 p2',p1=self.heatmap_size[0],p2=self.heatmap_size[1])
        ...
        if self.mlp_head_jacobian is not None:
            if self.jacobian_token:
                x_jacobian = self.to_keypoint_token(x[:, self.num_keypoints//2:self.num_keypoints])
            else:
            ...
            if self.affine_jacobian:
                ...
                cos_theta = theta[:,:,0:1]
                sin_theta = theta[:,:,1:2]
                rotate = torch.cat((cos_theta, -sin_theta, sin_theta, cos_theta),dim=-1)
                ...
                scale_x = scale[:,:,0:1]
                scale_y = scale[:,:,1:2]
                scale = torch.cat((scale_x, torch.zeros(scale_x.shape).to(x.device), torch.zeros(scale_x.shape).to(x.device), scale_y), dim=-1)
...
class TokenPose_S_base(nn.Module):
    def __init__(self, *, image_size, patch_size, num_keypoints, dim, depth, heads, mlp_dim, apply_init=False, apply_multi=True, hidden_heatmap_dim=64*6,heatmap_dim=64*48,heatmap_size=[64,48], channels = 3, dropout = 0., emb_dropout = 0.,pos_embedding_type="learnable",
                 estimate_jacobian=False, temperature=0.1, jacobian_token=True, hidden_dim=False, affine_jacobian=False):
        super().__init__()
