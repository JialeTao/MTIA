...
def animate_ddp(config, generator, kp_detector, checkpoint, log_dir, dataset, local_rank=-1):
    ...
    dataloader = DataLoader(dataset, batch_size=1, shuffle=False, num_workers=2, drop_last=False, sampler=train_sampler)
        ...
        with torch.no_grad():
            ...
            for frame_idx in range(driving_video.shape[2]):
                ...
                visualization = Visualizer(**config['visualizer_params']).visualize(source=source_frame,
                                                                                    driving=driving_frame, out=out, animate=True)
