...
class TokenPose_B(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        print(cfg.MODEL)
        ##################################################
        cfg_hg = cfg.model_params.kp_detector_params
        self.scale_factor = cfg_hg.scale_factor
        if self.scale_factor != 1:
            self.down = AntiAliasInterpolation2d(3,self.scale_factor)
        self.pre_feature= Hourglass(cfg_hg.block_expansion, in_features=3,max_features=cfg_hg.max_features, num_blocks=cfg_hg.num_blocks)
        self.transformer = TokenPose_TB_base(feature_size=[cfg.MODEL.IMAGE_SIZE[1]//4,cfg.MODEL.IMAGE_SIZE[0]//4],patch_size=[cfg.MODEL.PATCH_SIZE[1],cfg.MODEL.PATCH_SIZE[0]],
                                 ...
                                 channels=self.pre_feature.out_filters,
